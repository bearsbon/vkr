      ...
      <div key={user} className="employee">
        <EmployeeModal active={modalActive === index} setActive={setModalActive}>
          <div className="text-[#303439] ">
            <div className="content overflow-scroll overflow-x-hidden">
              <div className="left mx-auto">
                <img
                  src={user.avatar}
                  alt="ava"
                  class="mx-auto w-[250px] h-[250px] rounded-full object-cover object-top"
                />
                ...
                <div className="border__box">
                  ...
                  {user.workExp.map((c) => (
                    <div key={c.id} className="education__wrapper">
                      ...
                      <div>
                        <div className=" font-semibold text-lg text-[#2F89FE]">{c.location}</div>
                        <div className=" font-medium text-m">{c.workPosition}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="w-14 h-14 rounded-full ml-1 mr-5 border-[2px] border-sky-700 flex">
          <img
            className="employee_avatar m-auto w-[50px] h-[50px] object-cover object-top rounded-full"
            src={user.avatar}
            alt="ava"
          />
        </div>
        <div className="mt-1 col-start-2">
          <div className="name leading-4">{user.name.split(' ').slice(0, 2).join(' ')}</div>
          <span className="email">{user.email}</span>
        </div>

        <div className="text-center col-start-3">{user.position}</div>
        <div className="text-center lg:hidden xl:block 2xl:block col-start-4">
          {user.skills.map((skill) => (
            <span className="text-[12px] bg-orange-200 m-1 p-1 px-3 rounded-xl whitespace-nowrap">
              {skill}
            </span>
          ))}
        </div>
        ...
        <button
          className="bg-sky-700 text-white pl-5 pr-4 py-[0.4rem] rounded-[20px] ml-auto mr-5 active:translate-y-[1px] text-[14px] col-start-5"
          onClick={() => setModalActive(index)}>
          Подробнее
          <svg className="inline-block mx-2" width="9" height="13" viewBox="0 0 9 13" fill="none">
            <path
